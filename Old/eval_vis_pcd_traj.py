...
def chamfer_distance_RMSE(ref, est, max_error):
    ...
    rmse_chamfer_dist = 0.5 * rmse_dist1 + 0.5 * rmse_dist2
    ...
    return rmse_chamfer_dist, rmse_dist1, rmse_dist2, chamfer_dist, np.mean(dist1), np.mean(dist2)
...
def load_pred(pred_path, lidar_on_image_mask_ls=[], load_pcd=False):
    ...
    if 'tps' in pred_path:
        if not os.path.exists(f"{pred_path}/tps_fitted_points.npz"):
            ipdb.set_trace()
        print("Loading TPS fitted points...")
        data = np.load(f"{pred_path}/tps_fitted_points.npz")
        all_points = data['world_points_tps']
...
def run_icp_se3(src, tgt, voxel=0.1, max_corr=0.2, init=np.eye(4)):
    """
    ICP refinement (SE3). Returns 4x4 transform (src->tgt).
    """
...
def make_lines_colored_by_error(src_pts, tgt_pts, max_error=5.0, cmap_name="coolwarm"):
    assert src_pts.shape == tgt_pts.shape

    # Compute error (L2 distance)
    d = np.linalg.norm(src_pts - tgt_pts, axis=1)  # (N,)
    valid_mask = d <= max_error
    print(f"Valid correspondences: {np.sum(valid_mask)/len(d)}")
    d = d[valid_mask]
    src_pts = src_pts[valid_mask]
    tgt_pts = tgt_pts[valid_mask]
    N = src_pts.shape[0]
    d_norm = scale_to_range(d, 0.7, 1.0)
...
def vis_pred_for_drawing(pred_path, gt_path, R, t, s):
    vis = o3d.visualization.VisualizerWithKeyCallback()
    vis.create_window(window_name="Local0")
    opt = vis.get_render_option()
    opt.point_size = float(5.0)
    opt.light_on = False
    # Keyboard: N to continue; Q to quit (optional)
    ...
    vis1.create_window(window_name="Local1")
    opt = vis1.get_render_option()
    opt.point_size = float(5.0)
    opt.light_on = False
    # Keyboard: N to continue; Q to quit (optional)
    state = {"step": False, "quit": False}
    vis1.register_key_callback(ord("N"), lambda _: state.__setitem__("step", True))
    vis1.register_key_callback(ord("Q"), lambda _: state.__setitem__("quit", True))
    
    vis2 = o3d.visualization.VisualizerWithKeyCallback()
    vis2.create_window(window_name="World")
    opt = vis2.get_render_option()
    opt.point_size = float(5.0)
    opt.light_on = False
    # Keyboard: N to continue; Q to quit (optional)
    state = {"step": False, "quit": False}
    vis2.register_key_callback(ord("N"), lambda _: state.__setitem__("step", True))
    vis2.register_key_callback(ord("Q"), lambda _: state.__setitem__("quit", True))
    ...
    intrinsics = [np.loadtxt(os.path.join(gt_path, 'intrinsic', f"{cam_list[i]}.txt")) for i in range(len(cam_list))]

    # gt_cam2worlds = np.array([[np.loadtxt(os.path.join(gt_path, 'cam2world', f"{cam}/{frame:0>3d}.txt")) for frame in range(len(os.listdir(os.path.join(gt_path, 'cam2world', cam))))] for cam in cam_list[:3]]) # (C,T,4,4)
    # gt_cam2worlds_aligned = np.empty_like(gt_cam2worlds)
    # for cam_id in range(gt_cam2worlds.shape[0]):
    #     for frame_id in range(gt_cam2worlds.shape[1]):
    #         R_gt = gt_cam2worlds[cam_id, frame_id, :3, :3]
    #         t_gt = gt_cam2worlds[cam_id, frame_id, :3, 3]
    #         R_new = R @ R_gt
    #         t_new = s * (R @ t_gt) + t
    #         T = np.eye(4, dtype=gt_cam2worlds.dtype)
    #         T[:3, :3] = R_new
    #         T[:3, 3]  = t_new
    #         gt_cam2worlds_aligned[cam_id, frame_id] = T
    ...
    world_poses = []
    submap_num = len(glob.glob(f"{pred_path}/*.npz"))
    for submap_id in range(submap_num):
        data = np.load(f"{pred_path}/{submap_id:0>3d}.npz")
        world_points.append(data['world_points'])
        all_colors.append(data['color']/255)
        local_points.append(data['local_points'])
        local_poses.append(data['local_poses'])
        world_poses.append(data['world_poses'])
        
        if len(local_points) == 2:
            z_shift = [0, 0]
            
            for sid in range(len(local_poses)):
                data = o3d.geometry.PointCloud()
                data.points = o3d.utility.Vector3dVector(local_points[sid]+np.array([[0,0,z_shift[sid]]]))
                data.colors = o3d.utility.Vector3dVector(all_colors[sid])
                local_geometries_ls[sid].append(data)
                
                traj_line_list = []
                # camera_color_map = ['#A6CEE3', '#1F78B4', '#BEAED4']
                # def hex_to_rgb01(h):
                #     h = h.lstrip('#')
                #     return [int(h[i:i+2], 16) / 255.0 for i in (0, 2, 4)]
                # camera_color_map = [hex_to_rgb01(c) for c in camera_color_map]
                for cam_id in range(len(local_poses[sid])):
                    if cam_id // len(cam_list) == 1:
                        continue
                    if cam_id%len(cam_list) >= 3:
                        continue
                    cam2world = local_poses[sid][cam_id]
                    cam2world[:3, 3] += np.array([0,0,z_shift[sid]])
                    if cam_id%len(cam_list) == 0:
                        traj_line_list.append(cam2world) 
                    # Apply Sim(3)
                    local_geometries_ls[sid].extend(create_camera_frustum(cam2world, intrinsics[cam_id%len(cam_list)], img_size=(height, width), frustum_length=0.3, color=camera_color_map[cam_id % len(cam_list)]))

                max_range = 10
                data = o3d.geometry.PointCloud()
                mean = np.mean(world_points[sid], axis=0)
                valid_mask = np.linalg.norm(world_points[sid] - mean[None, :], axis=1) < max_range
                data.points = o3d.utility.Vector3dVector(world_points[sid][valid_mask])
                data.colors = o3d.utility.Vector3dVector(all_colors[sid][valid_mask])
                # world_geometries.append(data)

                traj_line_list = []
                for cam_id in range(len(world_poses[sid][-5:])):   
                    cam2world = world_poses[sid][cam_id]
                    if cam_id%len(cam_list) >= 3:
                        continue
                    # # Apply Sim(3)
                    # cam2world_aligned = np.eye(4)
                    # cam2world_aligned[:3, :3] = R_a @ cam2world[:3, :3]
                    # cam2world_aligned[:3, 3]  = s * (R_a @ cam2world[:3, 3]) + t_a
                    if cam_id%len(cam_list) == 0:
                        traj_line_list.append(cam2world)
                    world_geometries.extend(create_camera_frustum(cam2world, intrinsics[cam_id%len(cam_list)], img_size=(height, width), frustum_length=0.015, color=camera_color_map[sid]))

            # gt_idx = range((submap_id-1)*2, submap_id*2+4)
            # H = world_poses[0][0] @ np.linalg.inv(gt_cam2worlds_aligned[0][gt_idx[0]])
            # traj_line_list = []
            # for cam_id in range(len(gt_cam2worlds_aligned)):
            #     for frame in gt_idx:
            #         cam2world = gt_cam2worlds_aligned[cam_id][frame]
            #         cam2world = H @ cam2world
            #         traj_line_list.append(cam2world)
            #         world_geometries.extend(create_camera_frustum(cam2world, intrinsics[cam_id], img_size=(height, width), frustum_length=0.1, color=camera_color_map[2]))

            data = np.load(f"{pred_path}/align_vis/submap_{submap_id:0>2d}_to_{submap_id-1:0>2d}.npz")
            tgt = data['tgt']
            pred = data['pred']
            random_keep_line_index = np.random.choice(pred.shape[0], size=pred.shape[0]//50, replace=False) # 
            # Left group lines (src vs tgt before)
            lines = make_lines_colored_by_error(pred[random_keep_line_index], tgt[random_keep_line_index], max_error=max_range)
            # world_geometries.append(lines)

            vis.clear_geometries()
            for g in local_geometries_ls[0]:
                vis.add_geometry(g, reset_bounding_box=first)
            vis1.clear_geometries()
            for g in local_geometries_ls[1]:
                vis1.add_geometry(g, reset_bounding_box=first)
            vis2.clear_geometries()
            for g in world_geometries:
                vis2.add_geometry(g, reset_bounding_box=first)
            vis.poll_events()
            vis2.poll_events()
            vis.update_renderer()
            vis2.update_renderer()
            first = False  # Do not reset view after the first frame

            # —— Blocking wait (press N to continue / Q to quit), can be removed if not needed —— #
            state["step"] = False
            print(f"[{pred_path}: {sid}->{sid-1}] Press 'N' for next, 'Q' to quit.")
...
def align_rotation_global(pred_cam2world, gt_cam2world, weights=None):
    """
    pred_cam2world, gt_cam2world: (N,4,4)
    Already aligned trajectories with Sim(3), here only do global R and t alignment to match rotation
    Returns: 4x4 SE3 transform T, so that T @ pred_cam2world best matches gt_cam2world
    """
    pred = pred_cam2world[:, :3, :3]  # (N,3,3)
    gt   = gt_cam2world[:, :3, :3]
    ...
    # Accumulate Delta_t = R_gt * R_pred^T
    M = np.zeros((3,3))
    for i in range(N):
        M += weights[i] * (gt[i] @ pred[i].T)

    # SVD to find nearest rotation matrix
    U, S, Vt = np.linalg.svd(M)
    R = U @ Vt
    if np.linalg.det(R) < 0:  # Ensure right-handed coordinate system
        U[:, -1] *= -1
        R = U @ Vt

    # Translation: align centers after rotation alignment
    pred_centroid = pred_cam2world[:, :3, 3].mean(axis=0)
    gt_centroid   = gt_cam2world[:, :3, 3].mean(axis=0)
...
def voxel_indices(points: np.ndarray, voxel_size: float):
    """
    Compute the integer voxel indices for each point.
    """
    return np.floor(points / voxel_size).astype(np.int64)


def extract_mutual_voxel_points(points_pred: np.ndarray,
                                points_gt: np.ndarray,
                                voxel_size: float):
    """
    Keep only the points in pred and gt point clouds that fall within mutually shared voxels.

    Parameters:
        points_pred: (N1, 3) numpy array
        points_gt:   (N2, 3) numpy array
        voxel_size:  float, voxel edge length (same unit as point clouds)
    Returns:
        points_pred_filtered: (M1, 3) numpy array
        points_gt_filtered:   (M2, 3) numpy array
    """
    # Compute voxel indices for each point cloud
    vid_pred = voxel_indices(points_pred, voxel_size)
    vid_gt   = voxel_indices(points_gt, voxel_size)

    # Construct voxel sets
    set_pred = set(map(tuple, vid_pred))
    set_gt   = set(map(tuple, vid_gt))

    # Compute intersection of voxel sets
    mutual_voxels = set_pred & set_gt
    if len(mutual_voxels) == 0:
        print("[Warning] No mutual voxels found — clouds may not overlap.")
        return np.empty((0, 3)), np.empty((0, 3))

    # Use boolean masks to filter each point cloud
    mutual_set = set(mutual_voxels)
    mask_pred = np.array([tuple(v) in mutual_set for v in vid_pred])
    mask_gt   = np.array([tuple(v) in mutual_set for v in vid_gt])

    points_pred_filtered = points_pred[mask_pred]
    points_gt_filtered   = points_gt[mask_gt]

    return points_pred_filtered, points_gt_filtered
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_error", type=float, default=10, help="Truncation for distances (meters) in metrics")
     ...
     # ICP options
    parser.add_argument("--icp", action="store_true", help="Apply ICP refinement (SE3)")
    parser.add_argument("--icp_voxel", type=float, default=0.5, help="Voxel size for ICP downsampling")
    parser.add_argument("--icp_max_corr", type=float, default=1, help="ICP max correspondence distance (meters)")
    parser.add_argument("--icp_ptp", action="store_true", help="Use point-to-point ICP (default: point-to-plane)")

    args = parser.parse_args()

    if args.vis:
        # visualize(vis_GT(args.GT, args.pred, dataset='nuscenes' if 'nuscenes' in args.GT else 'waymo'), name="GT")
        # visualize(vis_pred(args.GT, args.pred, dataset='nuscenes' if 'nuscenes' in args.pred else 'waymo'), name="GT")
        visualize_groups([vis_pred(args.GT, args.pred, dataset='nuscenes' if 'nuscenes' in args.pred else 'waymo'), 
                          vis_pred(args.GT, args.pred.replace("pose+tps+v0.05e0.005", "sim3"), dataset='nuscenes' if 'nuscenes' in args.pred else 'waymo'),
                        #   vis_pred(args.GT, args.pred.replace("pose+tps+v0.05e0.005", "sl4"), dataset='nuscenes' if 'nuscenes' in args.pred else 'waymo'),
                        #   vis_pred(args.GT, args.pred.replace("VGGT", "Pi3"), dataset='nuscenes' if 'nuscenes' in args.pred else 'waymo'),
                        #   vis_pred(args.GT, args.pred.replace("VGGT", "Pi3").replace("pose+tps+v0.05e0.005", "sim3"), dataset='nuscenes' if 'nuscenes' in args.pred else 'waymo'),
                        #   vis_pred(args.GT, args.pred.replace("VGGT", "Pi3").replace("pose+tps+v0.05e0.005", "sl4"), dataset='nuscenes' if 'nuscenes' in args.pred else 'waymo'),
                          
                          ])
    
    if args.eval_traj or args.eval_pcd or args.vis_for_drawing:
        # pred_pcd, pred_cam2world = load_pred(args.pred, lidar_on_image_mask_ls, load_pcd=args.eval_pcd)
        ...
        ###############################
        if args.vis_for_drawing:
            R_a, t_a, s, _ = align_pred_cam2world_to_gt(pred_cam2world_ref, gt_cam2world_ref, visualize=args.vis_eval_traj, return_metrics=False, correct_heading=False)
            vis_pred_for_drawing(args.pred, args.GT, R_a, t_a, s)
        ...
        if args.eval_pcd:
            ...
            # # 3) Optional ICP refinement (SE3, no scale)
            # try:
            #     T_icp = run_icp_se3(
            #         src=pred_pcd, tgt=gt_pcd,
            #         voxel=args.icp_voxel,
            #         max_corr=args.icp_max_corr,
            #         init=np.eye(4)
            #     )
            #     pred_pcd.transform(T_icp)
            #     print(colored("[ICP] Applied rigid ICP refinement.", "cyan"))
            # except Exception as e:
            #     print(colored(f"[ICP] Failed to apply ICP refinement: {e}", "red"))



            # pred_pcd, gt_pcd = extract_mutual_voxel_points(
            #     points_pred=np.asarray(pred_pcd.points),  # 已经经过Sim(3)+ICP对齐
            #     points_gt=np.asarray(gt_pcd.points),
            #     voxel_size=args.max_error            # 例如 0.1m
            # )


            
            # 4) Metrics (Pred->GT accuracy, GT->Pred completeness, symmetric Chamfer)
            rmse_chamfer, rmse_acc, rmse_comp, chamfer, acc, comp = chamfer_distance_RMSE(
            ...
            print(colored("After ICP RMSE acc (Pred->GT):", "green"), rmse_acc)
            print(colored("After ICP RMSE comp (GT->Pred):", "green"), rmse_comp)
            print(colored("After ICP Chamfer (sym):", "green"), rmse_chamfer)
            ...
            if args.vis_eval_pcd:
                ...
                # vis.add_geometry(pred_pcd.translate((0,0,0)))
                # vis.run()
