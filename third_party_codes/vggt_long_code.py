...
'''
The following codes are adapted from VGGT Long (https://github.com/DengKaiCQ/VGGT-Long/blob/main/loop_utils/sim3utils.py)
'''
import os
import numpy as np
import trimesh
...
def robust_weighted_estimate_sim3(src, tgt, init_weights, delta=0.1, max_iters=20, tol=1e-9, use_scale=True):
    """
    src:  (Nx3)
    tgt:  (Nx3)
    init_weights:  (N,)
    """
    s, R, t = weighted_estimate_sim3(src, tgt, init_weights, use_scale=use_scale)
    ...
    for iter in range(max_iters):
        ...
        s_new, R_new, t_new = weighted_estimate_sim3(src, tgt, combined_weights, use_scale=use_scale)
